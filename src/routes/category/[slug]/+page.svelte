		...
		<div class="py-[50px] px-0">
			<Row>
				<Col className="basis-full max-w-full">
					<div>
						<h1 class="mb-[10px]">{$s_category.name} Articles</h1>
...
<section>
	<Container>
		<FilterBar current_page={$s_current_page} page_size={$s_page_size} posts_count={$s_posts_count} pages_count={$s_pages_count} goto_pre_link="category/{$s_category.slug}" />
		<Posts posts={$s_posts} />
	</Container>
</section>
